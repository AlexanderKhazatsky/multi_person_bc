...
class InfiniteBatchLoader:
	...
	def random_batch(self):
		...
		return ptu.from_numpy(obs),  ptu.from_numpy(acts)
...
class BCTrainer:
	def __init__(self, model=None, train_data=None, test_data=None, log_dir='', env_name=None, horizon=65, num_epochs=25,
			batch_size=32, weight_decay=0.0, lr=1e-3, num_eval_traj=5, video_logging_period=1):
	...
	def compute_loss(self, X, y, test=False):
		prefix = "test-" if test else "train-"
		bc_loss = self.model.compute_loss(X, y)
	...
	def evaluate_policy(self, epoch):
		...
		for j in range(self.num_eval_traj):
			...
			for i in range(self.horizon):
				...
				action = ptu.get_numpy(self.model(img)).flatten()
	...
	def test_batch(self, X, y):
		self.model.eval()
		loss = self.compute_loss(X, y, test=True)
	...
	def test_epoch(self, batches=10):
		start_time = time.time()
		for b in range(batches):
			X, y = self.test_dataset.random_batch()
			self.test_batch(X, y)
...
def make_datasets(filepath, num_traj_limit=300, train_percent=0.9):
	...
	train_dataset = {'observations': dataset['observations'][train_ind],
					'actions': dataset['actions'][train_ind]}
	test_dataset = {'observations': dataset['observations'][test_ind],
					'actions': dataset['actions'][test_ind]}
...
def bc_exp_launcher(variant, run_id, exp_id):
	...
	train_dataset, test_dataset = make_datasets(datapath, num_traj_limit=variant['demo_size'])
	trainer = BCTrainer(model=model, train_data=train_dataset, test_data=test_dataset,
		log_dir=log_dir, env_name=env_name, horizon=horizon)
	trainer.train()
