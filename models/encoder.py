...
class ImagePolicy(nn.Module):
    def __init__(
        ...
        for i in range(self.num_layers):
            if i == 0:
                in_dim, out_dim = self.representation_size, self.num_hiddens
    ...
    def compute_loss(self, obs, expert_actions):
        latent = self.process_image(inputs)
    ...
    def encode(self, obs):
        latent = self.process_image(inputs)
        return latent.reshape(-1, self.representation_size)

    def get_action(self, latent):
        latent = latent.reshape(-1, self.representation_size)
        for i in range(self.num_layers):
            latent = self.hidden_layers[i](self.relu(latent))
